//! Android gamepad event source (sokol/native).
//!
//! Detects controllers and distinguishes TV "d-pad remotes" via JNI
//! (InputDevice.getSources / SOURCE_GAMEPAD vs SOURCE_DPAD). Wave-0 stub:
//! returns no events.
//!
//! TODO(assembler#248): implement JNI-based hotplug detection. Fill in
//! `Source` below; do NOT touch ../gamepad_source/root.zig or build.zig.

const source = @import("root.zig");
const GamepadEvent = source.GamepadEvent;
const GamepadDescription = source.GamepadDescription;

pub const Source = struct {
    pub fn pollEvents(out: []GamepadEvent) usize {
        _ = out;
        return 0;
    }

    pub fn describe(out: []GamepadDescription) usize {
        _ = out;
        return 0;
    }
};
